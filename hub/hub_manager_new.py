...
class WildLinkHubManager:
    ...
    def on_message(self, client, userdata, msg):
        ...
        try:
            ...
            sys_id, vst_role, msg_type = parts[2], parts[3], parts[4]
            ...
            if msg_type == 'res':
                ...
                # 2. 【フェーズ2: 設定同期】 コマンド完了時に設定値があれば同期
                if (cmd_status == 'completed' or cmd_status == 'success'):
                    log_ext = payload.get('log_ext')
                    # 文字列で届いた場合の救済
                    if isinstance(log_ext, str):
                        try: log_ext = json.loads(log_ext)
                        except: log_ext = None

                    if isinstance(log_ext, dict):
                        logger.info(f"🔄 [Sync] '{vst_role}' reporting new config. Syncing...")
                        self.db.update_vst_configs(sys_id, vst_role, log_ext)

                # 3. コマンドのライフサイクル管理 (最終決着)
            ...
            # --- VSTからのイベント(event)処理 ---
                ...
                if act_db == 1 or act_db is True:
                    self.db.insert_node_data(sys_id, vst_role, payload)
    ...
    def _handle_command_lifecycle(self, cmd_id, status, payload):
        ...
        if status in ["completed", "success"]:
            ...
            final_status = "error"
            final_code = payload.get('log_code', 500)
            final_msg = payload.get('log_msg', 'Failed/Error reported')
